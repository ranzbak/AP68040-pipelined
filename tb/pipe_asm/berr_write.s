...
; does, so the store lands exactly once.  On an earlier micro-op, and on
; any MOVEM store, the instruction restarts (PC = the instruction, WB1S
; clear, WB3A = FA, WB3D = the data); a MOVEM sets SSW CM with EA = its
; calculated EA (M68040UM 8.4.6.2 p. 8-25, PLAN D15).  SSW RW = 0.  Frames go to the log;
...
start:
	...
	movea.l	#$7020,a2
	movem.l	d1-d3,(a2)	; $7028 (the last) faults: CM, the MOVEM restarts
	movea.l	#$7030,a3
	move.w	#$1234,(a3)
