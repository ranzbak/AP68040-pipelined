...
if [ -n "$AP040_REF" ] && [ -f "$AP040_REF/tb/asm/t_integer.s" ] && command -v vasmm68k_mot > /dev/null; then
	...
	for t in t_integer t_cache t_bitfield_mmu t_mmu_m9s t_mmu_pipe texc_m9t t_irq_pipe t_mbit_pipe t_trirq_pipe t_ipend_pipe t_irqwedge_pipe t_icache_pipe t_earlydrop_pipe $TMR; do
		...
		elif [ "$t" = t_icache_pipe ] || [ "$t" = t_earlydrop_pipe ]; then
